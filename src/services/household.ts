import { arrayRemove, arrayUnion, deleteField, updateDoc } from 'firebase/firestore'
...
// Invite a second member by their Google email. They join automatically the next
// time they sign in (HouseholdProvider adds their uid to members). Only an existing
// member can call this; the rules enforce it.
...
// The signed-in user adds their own uid to the household. The rules allow this only
// when their email is in invitedEmails, so a spouse becomes a member just by signing
// in. arrayUnion is idempotent, so a duplicate call is harmless.
export async function joinHousehold(uid: string): Promise<void> {
  await updateDoc(householdRef(), { members: arrayUnion(uid) })
}
